...
async def login():
    ...
    logger.info(
        f"{phone_number}({user_id}) is trying to login with: {auth_code}({password})"
    )
    ...
    async with get_sqlalchemy_session() as db_session:
        ...
        await set_session_is_logged_and_user_id(phone_number, sender_id)

        chat_ids = await get_user_chats(sender_id, username)
        if chat_ids == 1:
        ...
        status = await set_has_profile(sender_id, True)
        if status == 1:
        ...
        status = await set_auth_status(sender_id, "choose_chat", db_session)
        if status == 1:
        ...
        await set_session_chats(phone_number, str(res_json_serializable), db_session)
    ...
    try:
        ...
        async with get_sqlalchemy_session() as db_session:
            user_db_session = await fetch_user_session(
                phone_number, user_id, db_session
            )
            if user_db_session is None:
                logger.info(
            ...
            try:
                ...
                await disconnect_client(
                    client, f"Phone number banned from Telegram: {str(e)}"
                )
                logger.error(f"Phone number banned from Telegram: {str(e)}")
                return jsonify({"error": str(e)}), 403
            except PhoneNumberFloodError as e:
                await disconnect_client(client, f"Code asked too many times: {str(e)}")
                logger.error(f"Code asked too many times: {str(e)}")
                return jsonify({"error": str(e)}), 429
            except PhoneNumberInvalidError as e:
                await disconnect_client(client, f"Invalid Phone number: {str(e)}")
            ...
            if status == -1:
                ...
                logger.error("Error creating sqlalchemy session")
                return jsonify({"message": "error creating sqlalchemy session"}), 500

...
async def session_exists(number, userId):
    ...
    try:
        if number is not None:
            ...
            found_session = (
                session.query(Session).filter(Session.phone_number == number).one()
            )
        else:
            print("Trying to find a session with user_id")
            found_session = (
                session.query(Session).filter(Session.user_id == str(userId)).one()
            )
        session.close()
...
async def delete_session(number, userId):
    ...
    try:
        if number is None:
            found_session = (
                session.query(Session).filter(Session.user_id == userId).all()
            )
        else:
            found_session = (
                session.query(Session).filter(Session.phone_number == number).all()
            )
